...
def initBeta(S, b0, b1):
    Sset = list(set(S))
    Sset.sort()
    initbetas = rng.uniform(low=b0, high=b1, size=len(Sset))
    initbetas.sort()
    betaI = dict()
    ...
    for t in range(T):
        for d in range(D[t]):
            for n in range(N[t][d]):
                z_vec = rng.multinomial(1, [1.0 / K] * K)
...
def proposeBeta(beta_ori, epsilon, b2):
    beta_star = reflect(rng.uniform(beta_ori - epsilon, beta_ori + epsilon, 1)[0], b2, epsilon)
    return beta_star
...
def sampleZ(S, t, d, v, ndk, nikv, nik, K, V, betaI, alpha):
    ...
    k_vec_new = rng.multinomial(1, p_uni)
    k_new = int(np.where(k_vec_new == 1)[0])
    return k_new

def save_theta( d, ndk, K, alpha):
    p = [(ndk[d][k] + alpha[k]) for k in range(K)]
    p_uni = [i / sum(p) for i in p]
    return p_uni
...
def saveCp(path_output, iter, S, betaI):
    ...
    with open(path_output, 'a') as f:
        f.write("iter={}, n_cp={}, breaks={}, betas={}\n". \
                        format(iter, n_cp, ' '.join(map(str, breaks)), ' '.join(map(str, betas))))


def compute_pikv(betaI, nikv, nik, K, V):
    pikv = dict()
    for i in nikv.keys():
        pikv[i] = np.zeros(shape=(K, V))
        for k in range(K):
            for v in range(V):
                pikv[i][k][v] = (nikv[i][k][v] + betaI[i])/(nik[i][k] + V * betaI[i])
    return pikv

def savePhi(path_phi, betaI, pikv, revkv_vocab_dict, K, V):
    with open(path_phi, "w") as f:
        f.write("s,beta,k,vocab,prob\n")
        for i in pikv.keys():
            for k in range(K):
                for v in range(V):
                    line = "{},{},{},{},{}\n".format(i, betaI[i], k, revkv_vocab_dict[v], pikv[i][k][v])
                    f.write(line)

def savePhi_Top100(path_phi, betaI, pikv, revkv_vocab_dict, K, V):
    with open(path_phi, "w",encoding='utf-8') as f:
        f.write("s,beta,k,vocab,prob,vid\n")
        for i in pikv.keys():
            for k in range(K):
                v2p = [(v, p) for v, p in enumerate(pikv[i][k])]
                v2p_top100 = sorted(v2p, key=lambda x: x[1], reverse=True)[:100]
                for vid, (v, p) in enumerate(v2p_top100):
                    line = "{},{},{},{},{},{}\n".format(i, betaI[i], k, revkv_vocab_dict[v], pikv[i][k][v], vid+1)
                    f.write(line)
                    

def update_hyper(S,T):
    def f(b):
        f0 = -1 
        for t in range(T):
            nii = cal_nii(S, t)
            f0 = f0 + scipy.special.digamma(b[0]+b[1])+scipy.special.digamma(nii+b[0])-scipy.special.digamma(b[0])-scipy.special.digamma(1+nii+b[0]+b[1])
        f1 = -1
        for t in range(T):
            nii = cal_nii(S, t)
            f1 = f1 + 1/b[1]+scipy.special.digamma(b[0]+b[1])-scipy.special.digamma(1+nii+b[0]+b[1])
        return [f0, f1]
# Nearest root is [sqrt(2), 0]
    alpha0 = scipy.optimize.newton_krylov(f, [1,1], iter=100,f_tol = 0.01)[0]
    beta0 = scipy.optimize.newton_krylov(f, [1,1], iter=100,f_tol = 0.01)[1]
    return alpha0,beta0
