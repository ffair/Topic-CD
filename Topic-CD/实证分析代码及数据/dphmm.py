...
parser.add_argument('--experiment_dir', default='experiments/cell',
...
# io path
path_doc = os.path.join(args.experiment_dir, '1_docs.txt'.format(args.prefix))
path_vocab = os.path.join(args.experiment_dir, '{}_vocab.json'.format(args.prefix))
path_output = os.path.join(args.experiment_dir, 'output.txt')
...
for epoch in range(n_epoch):
    ...
    for iter in range(n_iter):
        ...
        def mapf_updatez(zlist):
            ...
            theta=[]
            for d in range(D[t]):
                theta.append(save_theta( d, ndk, K, alpha))
                for n in range(N[t][d]):
                    ...
                    nkv[k_new][v] += 1
                    nk[k_new] += 1
            return (zdn, vdn, ndk, nkv, nk,theta)
        ...
        def updateZ(T, Ztdn, Vtdn, ntdk, ntkv, ntk):
            ...
            res = list(map(mapf_updatez, zlist))
            Ztdn = [z for (z, v, n1, n2, n3,p) in res]
            Vtdn = [v for (z, v, n1, n2, n3,p) in res]
            ntdk = [n1 for (z, v, n1, n2, n3,p) in res]
            ntkv = [n2 for (z, v, n1, n2, n3,p) in res]
            ntk = [n3 for (z, v, n1, n2, n3,p) in res]
            theta_prob = [p for (z, v, n1, n2, n3,p) in res]
#            pool.close()
#            pool.join()
            return Ztdn, Vtdn, ntdk, ntkv, ntk, theta_prob


        Ztdn, Vtdn, ntdk, ntkv, ntk, theta_prob = updateZ(T, Ztdn, Vtdn, ntdk, ntkv, ntk)
        z_end = time.time()
